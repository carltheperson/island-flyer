...
public class Loader {
	...
	// Water
	public RawModel loadToVAO(float[] positions, int[] indices, ChunkModel chunkModel, float[] waterOffsets) {
		int vaoID = createVAO();
		bindIndicesBuffer(indices);
		chunkModel.setPositionsID(storeDataInAttributeList(0, 3, positions));
		chunkModel.setOffsetsID(storeDataInAttributeList(1, 1, waterOffsets));
		unbindVAO();
		return new RawModel(vaoID, indices.length);
	}
	...
	// Water
	public void updatePositionsAndOffsets(float[] positions, ChunkModel chunkModel, float[] waterOffsets) {
		// Positions
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, chunkModel.getPositionsID());
		FloatBuffer buffer = storeDataInFloatBuffer(positions);
		GL15.glBufferData(GL15.GL_ARRAY_BUFFER, buffer, GL15.GL_STATIC_DRAW);
		...
		// Offsets
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, chunkModel.getOffsetsID());
		buffer = storeDataInFloatBuffer(waterOffsets);
		GL15.glBufferData(GL15.GL_ARRAY_BUFFER, buffer, GL15.GL_STATIC_DRAW);
	...
	public int createEmptyVbo(int floatCount) {
		...
		vbos.add(vbo);
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vbo);
		GL15.glBufferData(GL15.GL_ARRAY_BUFFER, floatCount, GL15.GL_STREAM_DRAW);
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, 0);
		return vbo;
	}
	...
	public int loadTexture(String fileName) {
		...
		try {
			...
			if (GLContext.getCapabilities().GL_EXT_texture_filter_anisotropic) {
				...
				System.out.println("Anisotropic filter not supported - Loader class");
			}
		} catch (IOException e) {
